...
def nbrs_outside(node_id = 0, nbr_list = [], num_nodes = 1):
    count = 0
    for nbr_delta in nbr_list:
        if nbr_delta + node_id < 0  or nbr_delta + node_id >= num_nodes:
            count += 1
    return count
        
def try_rand_variation(grammar, vec_a, vec_b, subgraph_size = 2, max_attempts = 100):  
    node_types_a, neighbors_a = grammar.one_hot_to_graph(vec_a)
    node_types_b, neighbors_b = grammar.one_hot_to_graph(vec_b)
    
    if len(node_types_a) < subgraph_size + 1 or len(node_types_b) < subgraph_size + 1:
        return False

    for start_node_id in range(node_types_a - subgraph_size):   
        ntypes_subgraph_a = node_types_a[start_node_id: start_node_id + subgraph_size]
        nbrs_a = neighbors_a[start_node_id: start_node_id + subgraph_size]
        inside_a = [id for id in range(subgraph_size) if nbrs_outside(id, nbrs_a[id], subgraph_size) == 0]
        cut_a = [id for id in range(subgraph_size) if nbrs_outside(id, nbrs_a[id], subgraph_size) != 0]
        nbr_counts_outside  = map(lambda id: nbrs_outside(id, nbrs_a[id], subgraph_size), range(subgraph_size))
                
        if len(ntypes_inside_a) < 1 or len(ntypes_cut_a) < 1:
            continue
